...
import { Webhook, Plus, Trash2, Play, CheckCircle, XCircle, Clock, ToggleLeft, ToggleRight, Activity, RotateCw, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {webhooks.map((wh) => (
            <div key={wh.id} className="card hover:border-gray-700 transition-colors">
              <div className="flex items-start justify-between">
                <Link to={`/webhooks/${wh.id}`} className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 shrink-0 ml-3">
                  <Link to={`/webhooks/${wh.id}`} className="text-gray-500 hover:text-n0va-400 p-1" title="View details">
