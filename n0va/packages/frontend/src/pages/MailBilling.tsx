...
import { CreditCard, RefreshCw, AlertTriangle, TrendingUp, CheckCircle2, X, ChevronRight, Plus, Trash2, Zap } from "lucide-react";
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="card">
              ...
              <div className="text-xs text-gray-500 mt-1">${dash.priceMonthly}/mo · {dash.daysLeftInCycle} day(s) left in cycle</div>
            </div>
            <div className="card">
            ...
            <div className="card">
              ...
              <div className="text-xs text-gray-500 mt-1">{dash.invoices?.length || 0} invoice(s)</div>
            </div>
            <div className="card">
              ...
              <div className="space-y-2">
                ...
                {(dash.invoices || []).map((inv: any) => (
                  <div key={inv._id} className="flex items-center justify-between bg-gray-900/60 rounded-lg px-3 py-2">
                    <div className="min-w-0">
                      <div className="text-xs text-white">{inv.number}</div>
                      <div className="text-[10px] text-gray-500 truncate">
                        {(inv.lines || []).map((l: any) => l.description).join(" · ") || inv.status} · issued {String(inv.issuedAt || "").slice(0, 10)}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-xs font-semibold text-white">${Number(inv.total || 0).toFixed(2)}</span>
                      {inv.status === "open" ? (
                        <button className="btn-primary text-[10px] px-2 py-1" disabled={busy === "pay" + inv._id} onClick={() => payInvoice(inv._id)}>Pay</button>
                      ) : (
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">Paid</span>
                      )}
                    </div>
                  </div>
                ))}
