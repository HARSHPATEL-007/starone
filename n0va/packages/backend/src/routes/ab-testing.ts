...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (confidence > 90 && uplift > 0) {
      recommendation = `Variant shows ${uplift.toFixed(1)}% uplift with ${confidence.toFixed(1)}% confidence. Consider implementing the variant.`;
    } else if (confidence > 90 && uplift < 0) {
      recommendation = `Control outperforms variant by ${Math.abs(uplift).toFixed(1)}%. Stick with control.`;
    } else {
      recommendation = `Only ${confidence.toFixed(1)}% confidence. Continue test until 90%+ confidence is reached.`;
    }
    res.json({ pValue: parseFloat(pValue.toFixed(6)), confidence, bayesianProbability: parseFloat(bayesianProb.toFixed(4)), uplift: parseFloat(uplift.toFixed(2)), recommendation });
  })
);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const filtered = all.filter((t: any) => {
      if (type && t.testType !== type) return false;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!testName || !testType || !variants || !Array.isArray(variants) || variants.length < 2) {
      throw new AppError(400, "testName, testType, and variants array (min 2) required");
    }
    const enrichedVariants = variants.map((v: any, i: number) => ({
      id: v.id || `var_${v.name.toLowerCase().replace(/\s+/g, "_")}_${i + 1}`,
      name: v.name,
    ...
    if (isConnected()) {
      const test = await ABTest.create(data);
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (updates.status === "completed" || updates.variants) {
      let test: any;
      if (isConnected()) {
        test = await ABTest.findOne({ _id: id, tenantId }).lean();
      } else {
      ...
      if (variants && variants.length >= 2 && variants.every((v: any) => typeof v.impressions === "number" && typeof v.conversions === "number")) {
        ...
        updates.confidence = sig.confidence;
        updates.winner = sig.winner;
        updates.recommendation = ABTestService.generateRecommendation(variants, sig.winner, sig.confidence);
        updates.bayesianProbability = parseFloat(bayesianProb.toFixed(4));
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { id } = req.params;
    if (isConnected()) {
      const deleted = await ABTest.findOneAndDelete({ _id: id, tenantId });
      if (!deleted) throw new AppError(404, "AB test not found");
      return res.json({ message: "AB test deleted" });
    }
    ...
    res.json({ message: "AB test deleted" });
  })
);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (isConnected()) {
      ...
      if (variants.length >= 2) {
        ...
        test.bayesianProbability = parseFloat(bayesianProb.toFixed(4));
        test.pValues = sig.pValues;
        test.uplift = sig.uplift;
    ...
    if (variants.length >= 2) {
      const sig = ABTestService.computeSignificance(variants);
      const bayesianProb = ABTestService.computeBayesianProbability(variants[0], variants[1]);
      update.confidence = sig.confidence;
      update.winner = sig.winner;
      update.bayesianProbability = parseFloat(bayesianProb.toFixed(4));
      update.pValues = sig.pValues;
      update.uplift = sig.uplift;
      update.recommendation = ABTestService.generateRecommendation(variants, sig.winner, sig.confidence);
    }
