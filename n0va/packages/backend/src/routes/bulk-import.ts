...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      description: `Expected format for importing ${entityType}. Provide an array of objects with these fields.`,
    });
  })
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      if (!row.name) {
        errors.push({ row: i, error: "Missing required field: name" });
      }
    }
