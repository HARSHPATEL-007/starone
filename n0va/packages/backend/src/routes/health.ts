...
function computeHealthScore(campaign: any, metrics: any[]): { score: number; dimensions: Record<string, number>; issues: string[] } {
  ...
  return {
    score,
    dimensions: { budget: Math.round(budgetScore), performance: Math.round(performanceScore), efficiency: Math.round(efficiencyScore), engagement: Math.round(engagementScore) },
    issues,
  };
}
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const c of campaigns) {
      ...
      healthReports.push({ campaignId: c._id, campaignName: c.name, score, status: status, dimensions, issues });
    }
    ...
    res.json({
      averageScore: parseFloat((totalScore / n).toFixed(1)),
      counts,
      totalCampaigns: campaigns.length,
      topIssues,
      campaigns: healthReports,
    });
  })
);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (campaignDaily.length >= 2) {
      trend = campaignDaily.map((d: any) => {
        ...
        const dailyCtr = imp > 0 ? clicks / imp : 0;
        const dailyCvr = clicks > 0 ? conv / clicks : 0;
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        trend.push({ date: d.toISOString().split("T")[0], score: score });
      }
    }

    res.json({ campaignId: id, campaignName: campaign.name, score, status: categorize(score), dimensions, issues, trend });
  })
);
